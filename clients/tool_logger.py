...
def display_tool_call(tool_name: str, args: dict):
    """
    Display tool call information in Streamlit UI with REST endpoint.
    
    Shows an expandable section with:
    - MCP tool name
    - Tool arguments
    ...
    if not STREAMLIT_AVAILABLE:
        ...
        print(f"[ARGS] {args}")
        print(f"[REST API] {get_rest_api_url(tool_name, args)}")
        return
    
    if not should_show_logging():
        return
    
    rest_endpoint = get_rest_api_url(tool_name, args)
    
    with st.expander("🔧 Tool Call Details", expanded=False):
        col1, col2 = st.columns([1, 2])
        with col1:
            st.markdown("**MCP Tool:**")
